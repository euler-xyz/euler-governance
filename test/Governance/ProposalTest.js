...
const Timelock = artifacts.require('Timelock');
  ...
  describe('Goverance#propose/5', () => {
    ...
    xit("Given the sender's GetPriorVotes for the immediately
    previous block is above the Proposal Threshold (e.g. 2%),
    the given proposal is added to all proposals, given the following
    settings", async () => {
    ...
    describe("simple initialization", () => {
      ...
      it("End block is set to the current block number plus
      the sum of vote delay and vote period", async () => {
      ...
      describe("This function must revert if", () => {
        it("the length of the values, signatures or calldatas
        arrays are not the same length,", async () => {
        ...
        describe("Additionally, if there exists a pending or
        active proposal from the same proposer, we must revert.", () => {
          it("reverts with pending", async () => {
            await shouldFailWithMessage(
              gov.propose(targets, values, signatures, callDatas, "do nothing"),
            "revert Governance::propose: one live proposal per
            proposer, found an already active proposal");
          });
          ...
          it("reverts with active", async () => {
            ...
            await shouldFailWithMessage(
              gov.propose(targets, values, signatures, callDatas, "do nothing"),
            "Governance::propose: one live proposal per proposer,
            found an already active proposal");
          });
        });
      });

      it("This function returns the id of the newly created
      proposal. # proposalId(n) = succ(proposalId(n-1))", async () => {
        ...
        await mineBlock();
        let nextProposal = await gov.propose(targets, values,
          signatures, callDatas, "yoot", { from: accounts[2].address });
        let nextProposalId = await gov.latestProposalIds(accounts[2].address);

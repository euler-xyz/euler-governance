...
describe('Goverance contract: proposals', () => {
  ...
  describe('simple initialization', () => {
    ...
    it('End block is set to the current block number plus the sum of vote delay and vote period', async () => {
      ...
      expectBignumberEqual(trivialProposal.endBlock, `${proposalBlock + 1 + 20}`);
    });
...
describe('Goverance#propose/5', () => {
  ...
  before(async () => {
    ...
    timelock = await Timelock.new(owner, 86400 * 2, {from: owner});

    // Deploy Euler governance token contract
    eul = await eulerTokenInstance.new(owner, {from: owner});

    // Deploy Governance contract
    gov = await govInstance.new(address(0), eulerTokenInstance.address, address(0), {from: owner});
  });
  ...
  before(async () => {
    ...
    await eulerTokenInstance.delegate(owner);
    await govInstance.propose(targets, values, signatures, callDatas, 'do nothing');
    proposalBlock = await web3.eth.getBlockNumber();
    proposalId = await govInstance.latestProposalIds(owner);
    trivialProposal = await govInstance.proposals(proposalId);
  });

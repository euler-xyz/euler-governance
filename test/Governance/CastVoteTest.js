...
const EIP712 = require('../Utils/EIP712');
  ...
  describe("Goverance#castVote/2", () => {
    ...
    describe("We must revert if:", () => {
      it("There does not exist a proposal with matching proposal
      id where the current block number is between the proposal's
      start block (exclusive) and end block (inclusive)", async () => {
    ...
    describe("Otherwise", () => {
      it("we add the sender to the proposal's voters set", async () => {
        let receipt1 = await gov.getReceipt(proposalId, accounts[2].address);
        expect(receipt1.hasVoted).to.be.equal(false);
        await gov.castVote(proposalId, true, { from: accounts[2].address });
        let receipt2 = await gov.getReceipt(proposalId, accounts[2].address);
        expect(receipt2.hasVoted).to.be.equal(true);
      });

      describe("and we take the balance returned by GetPriorVotes for
      the given sender and the proposal's start block,
      which may be zero,", () => {
      ...
      it("receipt uses one load", async () => {
        let actor = accounts[2].address;
        let actor2 = accounts[3].address;
        await enfranchise(eul, actor, parseEther('400001'));
